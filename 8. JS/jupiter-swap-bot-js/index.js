...
// Global variables to track current purchase and API server status
...
class TradingConfig {
  constructor(tokenMint, solAmount, stopLossPercentage, takeProfitPercentage, slippageBps, apiPort, apiKey) {
    this.tokenMint = tokenMint;         // SPL token to trade
    this.solAmount = solAmount;         // Amount of SOL to use for purchase (in lamports)
    this.stopLossPercentage = stopLossPercentage;   // Stop loss as a percentage (e.g., 0.35 = 35% loss)
    ...
    this.apiPort = apiPort;             // Port for the API server
...
function loadConfigFromEnv() {
  try {
    ...
    // Get stop loss percentage - default to 35% (0.35)
    const stopLossStr = process.env.STOP_LOSS_PERCENTAGE || '0.35'; // Default to 35%
    ...
    // Get API port
    const apiPortStr = process.env.API_PORT || '3000'; // Default to port 3000
    const apiPort = parseInt(apiPortStr, 10);
    if (isNaN(apiPort)) {
      throw new Error(`Invalid API port: ${apiPortStr}`);
    }
...
// Sell tokens for SOL
async function sellTokens(connection, keypair, purchase, slippageBps, sellReason) {
  try {
    ...
    const priceResponse = await getOrder(priceCheckRequest);
    const solAmount = parseFloat(priceResponse.outAmount) / 1e9;
    const tokenAmount = Number(purchase.purchaseAmount);
    const currentPrice = tokenAmount / solAmount;
    ...
    try {
      ...
      // Reset the current purchase since we've sold
...
async function monitorPriceAndSell(connection, keypair, purchase, slippageBps) {
  try {
    // Set the current purchase in the global variable
    ...
    console.log(`OR will sell when triggered via API endpoint`);
    console.log(`Using entire purchased amount (${purchase.purchaseAmount} tokens) for monitoring`);
    console.log('=====================================\n');
    
    // Monitoring loop
    while (true) {
      // Check if API triggered selling
      if (sellSignal) {
        await sellTokens(connection, keypair, purchase, slippageBps, 'API SIGNAL TRIGGERED');
        sellSignal = false; // Reset the signal
        break;
      }
      ...
      try {
        ...
        if (shouldSell) {
          await sellTokens(connection, keypair, purchase, slippageBps, sellReason);
          break;
        }
      } catch (error) {
...
function setupApiServer(config) {
  ...
  // Route to trigger sell
  app.post('/api/sell', checkApiKey, (req, res) => {
    if (!currentPurchase) {
      return res.status(400).json({ error: 'No active purchase to sell' });
    }
    
    sellSignal = true;
    console.log('\n===== API TRIGGERED SELL SIGNAL =====');
    console.log('Sell signal received from API');
    console.log('Will execute sell on next monitoring cycle');
    console.log('=======================================\n');
    
    return res.json({ success: true, message: 'Sell signal received' });
  });
  ...
  app.get('/api/status', checkApiKey, (req, res) => {
    if (currentPurchase) {
      return res.json({
        status: 'active',
        tokenMint: currentPurchase.tokenMint.toString(),
        purchaseAmount: currentPurchase.purchaseAmount.toString(),
        purchasePrice: currentPurchase.purchasePrice
      });
    } else {
      return res.json({ status: 'inactive', message: 'No active purchase' });
    }
  });
  ...
  const server = app.listen(config.apiPort, () => {
    ...
    console.log(`Use endpoint POST /api/sell with header 'x-api-key: ${config.apiKey}' to trigger a sell`);
    console.log(`Use endpoint GET /api/status with the same header to check current status`);
    console.log('============================\n');
  });
...
async function main() {
  ...
  try {
    ...
    // Start API server
    apiServer = setupApiServer(config);
